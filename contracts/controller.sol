...
contract Controller is Enabled {
  ...
  function createGroup(string _name, string _avatar) external {
    ...
    People(ContractProvider(CMC).contracts("people-storage")).addGroup(address(_group), msg.sender, msg.sender);
  }
  ...
  function acceptMembership(address _group, address _user) external {
    ...
    People(ContractProvider(CMC).contracts("people-storage")).addGroup(_group, _user, msg.sender);
  }
